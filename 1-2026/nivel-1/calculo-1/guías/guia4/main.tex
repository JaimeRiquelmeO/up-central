...
\usepackage{tikz}
\usetikzlibrary{patterns, shadows.blur, fadings, calc, arrows.meta, decorations.pathreplacing, babel}
\usepackage{array}
\usepackage{colortbl}
...
\begin{solbox}{I. Soluciones — Trigonometría Aplicada}
\begin{enumerate}[label=\color{azulFuerte}\bfseries\arabic*., leftmargin=*, itemsep=0.6em]
    \item La altura del árbol es $\frac{10\sqrt{3}}{3}\text{ m} \approx 5{,}77\text{ m}$.
    \item La altura es $h_c = \sqrt{3} \approx 1{,}73$.
    \item La altura del cerro es $h = \frac{\sqrt{3} + 1}{4}\text{ km} = 250(\sqrt{3} + 1)\text{ m} \approx 683\text{ m}$.
    \item La longitud del corte diagonal es $y = 2\sqrt{3}'' \approx 3{,}46''$.
    \item $\tan(\theta)\cot(\theta) - \cos^2(\theta) = 1 - \cos^2(\theta) = \sen^2(\theta)$. Se cumple para todo $\theta \neq \frac{k\pi}{2}$ con $k \in \mathbb{Z}$.
    \item $\sen(\theta)(\cot(\theta) + \tan(\theta)) = \sen(\theta)\left(\frac{\cos\theta}{\sen\theta} + \frac{\sen\theta}{\cos\theta}\right) = \frac{\sen^2\theta + \cos^2\theta}{\cos\theta} = \frac{1}{\cos\theta} = \sec(\theta)$. Se cumple para todo $\theta \neq \frac{k\pi}{2}$ con $k \in \mathbb{Z}$.
\end{enumerate}
\end{solbox}
