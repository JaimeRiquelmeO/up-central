% ============================================================
%  PAUTA PEP 1 — FISICA I PARA INGENIERIA (FORMA B) — USACH
%  Usach Premium
%  Compila con: pdflatex pauta_pep1_forma_b.tex  (2 veces)
% ============================================================
...
\newcommand{\guiasnumero}{Pauta PEP 1 --- Forma B}
...
\newcommand{\rp}{\color{verdePaso}\bfseries}
...
\begin{center}
    ...
    {\Large Forma B \quad$\bullet$\quad C\'od: 10103-10140} \\[1.2cm]
...
$V_{I0}=70\,\mathrm{m/s}$ (solo gravedad) hasta detenerse. \textbf{Etapa II:}
motores con $a_{II}=5\,\mathrm{m/s^2}$ hasta $h=1000\,\mathrm{m}$. \textbf{Etapa III:}
solo gravedad hasta la altura m\'axima. \textbf{Etapa IV:} frenado uniforme desde
$H=300\,\mathrm{m}$ hasta el reposo al tocar el suelo. Se toma $\hat{\jmath}$ hacia arriba.
\end{enunciadobox}
...
\[
0=V_{I0}^2-2g\,y_I \;\Rightarrow\; y_I=\frac{70^2}{2(9{,}8)}=250\ \mathrm{m}.
\]
\textbf{Etapa II} (de $y_I=250$ con $v=0$ hasta $h=1000$, $a_{II}=5$):
\[
v_{II}^2=0+2(5)(1000-250)=7500 \;\Rightarrow\; v_{II}=\sqrt{7500}\approx 86{,}60\ \mathrm{m/s}.
\]
\textbf{Etapa III} (solo gravedad, sube hasta detenerse):
\[
y_{\max}=h+\frac{v_{II}^2}{2g}=1000+\frac{7500}{2(9{,}8)}=1000+382{,}65 .
\]
\begin{resultado}\centering
$\displaystyle y_{\max}\approx 1382{,}65\ \mathrm{m}$
\end{resultado}
\end{paso}

% ------------------- P2 (b) -------------------
\begin{paso}{2.b: Velocidad $\vec{v}_{IV}$ al inicio de la etapa final}
Usando el dato $y_{\max}=1382{,}65\,\mathrm{m}$, el cohete \textbf{cae libremente}
desde el reposo en $y_{\max}$ hasta $H=300\,\mathrm{m}$ (solo gravedad):
\[
v_{IV}^2=2g\,(y_{\max}-H)=2(9{,}8)(1382{,}65-300)=21219{,}94 .
\]
\[
v_{IV}=\sqrt{21219{,}94}\approx 145{,}67\ \mathrm{m/s}\ \ (\text{hacia abajo}).
\]
\begin{justif}{Forma anal\'itica (vector)}
El movimiento es descendente, por lo que la velocidad apunta en $-\hat{\jmath}$.
\end{justif}
\begin{resultado}\centering
$\displaystyle \vec{v}_{IV}\approx -145{,}67\,\hat{\jmath}\ \ \mathrm{m/s}$
\end{resultado}
\end{paso}

% ------------------- P2 (c) -------------------
\begin{paso}{2.c: Aceleraci\'on $\vec{a}_{\mathrm{final}}$ en el aterrizaje}
Usando el dato $v_{IV}=145{,}67\,\mathrm{m/s}$, el cohete frena de forma uniforme
desde $H=300\,\mathrm{m}$ hasta quedar en reposo ($v=0$) al tocar el suelo:
\[
0=v_{IV}^2-2\,a_{\mathrm{final}}\,H
\;\Rightarrow\;
a_{\mathrm{final}}=\frac{v_{IV}^2}{2H}=\frac{145{,}67^2}{2(300)}\approx 35{,}37\ \mathrm{m/s^2}.
\]
...
\end{justif}
\begin{resultado}\centering
$\displaystyle \vec{a}_{\mathrm{final}}\approx +35{,}37\,\hat{\jmath}\ \ \mathrm{m/s^2}$
\end{resultado}
\end{paso}
...
$\vec{A}$: $90\,\mathrm{m}$ a $37^\circ$ al Norte del Este.\quad
$\vec{B}$: $70\,\mathrm{m}$ a $53^\circ$ al Norte del Oeste.\quad
$\vec{C}$: tramo desconocido.\quad
Tras $\vec{A}+\vec{B}+\vec{C}$ el dron queda en
$\vec{D}$: $40\,\mathrm{m}$ al Sur de la base. Ejes: $+x$ Este, $+y$ Norte.
\end{enunciadobox}
...
\[
A_x=90\cos 37^\circ\approx 71{,}88,\qquad A_y=90\sin 37^\circ\approx 54{,}16 .
\]
\textbf{$\vec{B}$} ($53^\circ$ desde el Oeste hacia el Norte $\Rightarrow$ $x<0$, $y>0$):
\[
B_x=-70\cos 53^\circ\approx -42{,}13,\qquad B_y=70\sin 53^\circ\approx 55{,}90 .
\]
...
\]
\begin{resultado}\centering
$\vec{A}\approx(71{,}88,\,54{,}16)\,\mathrm{m}\quad
 \vec{B}\approx(-42{,}13,\,55{,}90)\,\mathrm{m}\quad
 \vec{D}=(0,\,-40)\,\mathrm{m}$
\end{resultado}
\end{paso}
...
\]
\begin{resultado}\centering
$\displaystyle \vec{C}\approx(-29{,}75,\,-150{,}07)\ \mathrm{m}$
\end{resultado}
\end{paso}
...
\textbf{Magnitud:}
\[
\lVert\vec{C}\rVert=\sqrt{(-29{,}75)^2+(-150{,}07)^2}\approx 152{,}99\ \mathrm{m}.
\]
\textbf{Direcci\'on:} ambas componentes son negativas $\Rightarrow$ tercer cuadrante
(Sur-Oeste). El \'angulo respecto a $+x$ es
\[
\theta=\arctan\!\left(\frac{-150{,}07}{-29{,}75}\right)
\;\Rightarrow\;
\theta\approx -101{,}21^\circ\ \equiv\ 258{,}79^\circ\ (\text{medido CCW desde }+x).
\]
\begin{justif}{Interpretaci\'on}
Equivale a $\approx 78{,}79^\circ$ al Sur del Oeste: el dron debe volver hacia el
suroeste para cerrar el per\'imetro.
\end{justif}
\begin{resultado}\centering
$\displaystyle \lVert\vec{C}\rVert\approx 152{,}99\ \mathrm{m}\qquad
\theta\approx 258{,}79^\circ\ \text{(o } -101{,}21^\circ\text{) respecto a }+x$
\end{resultado}
\end{paso}
...
\item[\textbf{P1 (c)}] $v_0\approx 6{,}91\,\mathrm{m/s}$;\quad $x^{*}\approx 104{,}70\,\mathrm{m}$ (en $t^{*}\approx 8{,}36\,\mathrm{s}$)
\item[\textbf{P2 (a)}] $y_{\max}\approx 1382{,}65\,\mathrm{m}$
\item[\textbf{P2 (b)}] $\vec{v}_{IV}\approx -145{,}67\,\hat{\jmath}\,\mathrm{m/s}$
\item[\textbf{P2 (c)}] $\vec{a}_{\mathrm{final}}\approx +35{,}37\,\hat{\jmath}\,\mathrm{m/s^2}$
\item[\textbf{P3 (a)}] $\vec{A}\approx(71{,}88,\,54{,}16)$;\ $\vec{B}\approx(-42{,}13,\,55{,}90)$;\ $\vec{D}=(0,\,-40)\,\mathrm{m}$
\item[\textbf{P3 (b)}] $\vec{C}\approx(-29{,}75,\,-150{,}07)\,\mathrm{m}$
\item[\textbf{P3 (c)}] $\lVert\vec{C}\rVert\approx 152{,}99\,\mathrm{m}$;\quad $\theta\approx 258{,}79^\circ\ (-101{,}21^\circ)$
\end{itemize}
\end{tcolorbox}
